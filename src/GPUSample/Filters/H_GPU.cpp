...
Image* Filters::H_GPU::Filter(Image* sourceImageOne) {
	...
	c = inFile2.peek();
	std::string output2;
	inFile2.open("Filters/equalization.cl");
	if (!inFile2) {
		std::cout << "Unable to open file";
		exit(1); // terminate with error
	}
	...
	// cdf-norm
	for (int i = 0; i < 256; i++) { CDF_Norm[i] = min((max(round(((CDF[i] - CDFmin) * 255) / ((imgHeight *imgWidth) - CDFmin)), 0)), 255); }
	...
	if (m_Context.BuildKernels(kernelProgram2, kernels)) {
		...
		cl::Buffer bufCumu((cl::Context)m_Context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, cl::size_type(Size * sizeof(cl_int)), &CDF_Norm);
