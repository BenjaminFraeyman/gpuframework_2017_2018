#include "Invert_GPU.h"

Image* Filters::Invert_GPU::Filter(Image* sourceImageOne)
{
	...
	std::string kernelProgram(
		"       __kernel void invert(__global const int* inputImage, __global int* outputImage, __global const float* randomTable, int randomTableSize){		"
		""
		...
		""
		"		red   = ((int)(255 * red) << 16) & 0xFF0000;"						// multiply each channel with the random factor, then shift back into position
		"		green = ((int)(255 * green) << 8)  & 0x00FF00;"
		"		blue  = ((int)(255 * blue) << 0)  & 0x0000FF;"
		""
		"		int alpha = (((color & 0xFF000000) >> 24) << 24) & 0xFF000000;"											// use maximum alpha (for output color)
	...
	if (m_Context.BuildKernels(kernelProgram, kernels))
	{
		...
		// define functor
		auto			  invert = cl::KernelFunctor< cl::Buffer&, cl::Buffer&, cl::Buffer&, cl_int>(kernels, "invert");

		// execute functor ( == kernel invert) with input arguments and output argument 
		cl::Event event = invert(cl::EnqueueArgs(rangeGlobal, rangeLocal), bufferIn, bufferOut, bufferRnd, randomTableSize, result);
