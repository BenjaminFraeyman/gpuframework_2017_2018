#include "ED_GPU_Global.h"

Image* Filters::ED_GPU_Global::Filter(Image* sourceImageOne) {
	...
	std::string kernelProgram(
		"       __kernel void ED_G(__global const int* inputImage, __global int* outputImage, __global const int* filter, __global const int* filter2){"
		""
		...
		"		int color = inputImage[get_global_id(1) * get_global_size(0) + get_global_id(0)];"
	...
	if (m_Context.BuildKernels(kernelProgram, kernels)) {
		...
		// define functor
		auto ED_G = cl::KernelFunctor< cl::Buffer&, cl::Buffer&, cl::Buffer&, cl::Buffer&>(kernels, "ED_G");

		// execute functor ( == kernel ED) with input arguments and output argument 
		cl::Event event = ED_G(cl::EnqueueArgs(rangeGlobal, rangeLocal), bufferIn, bufferOut, bufA, bufB, result);
