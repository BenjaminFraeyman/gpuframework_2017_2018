...
Image* Filters::H_GPU::Filter(Image* sourceImageOne) {
	...
	if (m_Context.BuildKernels(kernelProgram, kernels)) {
		...
		cl::Buffer bt((cl::Context)m_Context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR, cl::size_type(pixelsInBytes), pixelDataDestination);			// no upload!
		...
		//cl::Buffer bufHist((cl::Context)m_Context, CL_MEM_READ_WRITE | CL_MEM_USE_HOST_PTR, cl::size_type(Size * sizeof(size_t)), &Histogram);

		//char *mappedPointerIn = (char*)clQueue.enqueueMapBuffer(bufHist, CL_TRUE, CL_MAP_WRITE, 0, Size, 0, 0, &result);	// explicit upload
		//
		//clQueue.finish();
		//
